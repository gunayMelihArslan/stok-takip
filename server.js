...
app.post('/api/transactions/bulk', auth, async (req, res) => {
  ...
  for (const item of items) {
    ...
    const r = await query('INSERT INTO transactions(user_id,product_id,company,quantity,notes,tx_type) VALUES($1,$2,$3,$4,$5,$6) RETURNING id',
      [req.user.id, Number(item.product_id), company, qty, `${machine.machine_name} — toplu alım${notes ? ' — ' + notes : ''}`, 'out']);
...
app.get('/api/export/purchases', auth, async (req, res) => {
  const prs = (await query('SELECT p.*, u.display_name as requester_name, t.title as task_title FROM purchase_requests p LEFT JOIN users u ON p.requester_id = u.id LEFT JOIN tasks t ON p.task_id = t.id ORDER BY p.created_at DESC')).rows;
  const e = v => { const s = String(v || ''); return s.includes(',') || s.includes('"') ? `"${s.replace(/"/g, '""')}"` : s; };
  res.setHeader('Content-Type', 'text/csv; charset=utf-8');
