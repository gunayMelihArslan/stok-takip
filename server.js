...
app.get('/api/lots/counts', auth, async (req, res) => {
  ...
  const map = {};
  rows.forEach(r => { map[r.product_id] = parseInt(r.cnt); });
  res.json(map);
});
app.post('/api/products/:id/lots', auth, admin, async (req, res) => {
  const { production_year, quantity, notes } = req.body;
  if (!production_year) return res.status(400).json({ error: 'Üretim yılı zorunlu' });
  const qty = parseFloat(quantity);
  if (isNaN(qty) || qty < 0) return res.status(400).json({ error: 'Geçersiz miktar' });
  await query(
    `INSERT INTO product_lots(product_id,production_year,quantity,notes) VALUES($1,$2,$3,$4)
...
async function syncLotTotal(product_id) {
  const numCol = await getNumCol();
  if (!numCol) return;
  const row = (await query('SELECT COALESCE(SUM(quantity),0) as t FROM product_lots WHERE product_id=$1', [product_id])).rows[0];
  const prod = (await query('SELECT values FROM products WHERE id=$1', [product_id])).rows[0];
  if (!prod) return;
  const vals = { ...(prod.values || {}), [numCol.id]: String(parseFloat(row.t)) };
  await query('UPDATE products SET values=$1,updated_at=NOW() WHERE id=$2', [JSON.stringify(vals), product_id]);
}
...
async function restoreToLots(product_id, qty) {
  ...
  if (lot) { await query('UPDATE product_lots SET quantity=quantity+$1 WHERE id=$2', [qty, lot.id]); await syncLotTotal(product_id); }
}
