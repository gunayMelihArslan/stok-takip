...
const ATTENDANCE_LABELS = {
  ...
  overtime:    'Ek Mesai',
  annual:      'Senelik İzin',
  early_exit:  'Erken Çıktı',
  on_road:     'Yolda',
  absent:      'Gelmedi',
  sick:        'Raporlu',
  holiday_cel: 'Bayram',
  late:        'Geç Geldi',
  official_hol:'Resmi Tatil',
  road_permit: 'Yol İzni',
};

// Get monthly attendance for all personnel (admin) or self (personnel)
...
// Upsert a single day for a user (admin only)
app.post('/api/attendance', auth, admin, async (req, res) => {
  const { user_id, date, status, extra_hours, note_time, notes } = req.body;
  if (!user_id || !date || !status) return res.status(400).json({ error: 'user_id, date, status zorunlu' });
  if (!ATTENDANCE_LABELS[status]) return res.status(400).json({ error: 'Geçersiz durum' });

  await query(`
    INSERT INTO attendance(user_id, date, status, extra_hours, note_time, notes, created_by)
    VALUES($1, $2, $3, $4, $5, $6, $7)
    ON CONFLICT(user_id, date)
    DO UPDATE SET status=$3, extra_hours=$4, note_time=$5, notes=$6, updated_at=NOW()
  `, [user_id, date, status, extra_hours || null, note_time || null, notes || '', req.user.id]);

  broadcast('attendance_update', { user_id, date });
  res.json({ ok: true });
});

// Bulk upsert - mark whole month or multiple days
app.post('/api/attendance/bulk', auth, admin, async (req, res) => {
  const { records } = req.body; // [{user_id, date, status, ...}]
  if (!Array.isArray(records) || !records.length) return res.status(400).json({ error: 'Kayıt listesi boş' });
  for (const r of records) {
    if (!r.user_id || !r.date || !r.status) continue;
    await query(`
      INSERT INTO attendance(user_id, date, status, extra_hours, note_time, notes, created_by)
      VALUES($1, $2, $3, $4, $5, $6, $7)
      ON CONFLICT(user_id, date)
      DO UPDATE SET status=$3, extra_hours=$4, note_time=$5, notes=$6, updated_at=NOW()
    `, [r.user_id, r.date, r.status, r.extra_hours || null, r.note_time || null, r.notes || '', req.user.id]);
  }
...
// Summary stats for a user in a month
app.get('/api/attendance/summary', auth, async (req, res) => {
  ...
  const params = [year, month];
  let filter = '';
  if (uid) { params.push(uid); filter = ` AND user_id = $${params.length}`; }
  ...
  const rows = (await query(`
    SELECT
      ...
      status,
      COUNT(*) as count,
      SUM(CASE WHEN status='overtime' THEN COALESCE(extra_hours,0) ELSE 0 END) as total_overtime
    FROM attendance
    WHERE EXTRACT(YEAR FROM date)=$1 AND EXTRACT(MONTH FROM date)=$2 ${filter}
